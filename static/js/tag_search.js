document.addEventListener('DOMContentLoaded', function () {
    layui.use(['layer', 'flow'], function () {
        ...
        // 当前tag内容
        const tag = document.getElementById('hidden-div').querySelector('#tag').value;
        ...
        flow.load({
            elem: '#show_blogs', // 流加载容器
                ...
                // page会自动从1开始，每次递增1
                fetchBlogList(tag, (page - 1) * count, count, next);
            }
        });
        // 获取博客列表
        async function fetchBlogList(tag, start = 0, count = 10, next) {
            try {
                // 发送请求获取博客列表
                const response = await fetch(`/tag_search_list?tag=${tag}&start=${start}&count=${count}`);
                // 判断响应是否成功
                if (!response.ok) {
